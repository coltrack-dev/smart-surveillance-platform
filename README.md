...
- Признать известную проблему можно через `POST /api/v1/recordings/storage/s3/problems/acknowledge`; orphan внутри настроенного prefix удаляется вместе со всеми версиями через `POST /api/v1/recordings/storage/s3/problems/delete-orphan`. Для version-aware удаления ключу нужны `ListBucketVersions` и `DeleteObjectVersion`.
- История ручных и автоматических запусков доступна через `GET /api/v1/recordings/storage/cleanup/history`, детали запуска — через `GET /api/v1/recordings/storage/cleanup/history/{runId}/items`. Автоматическая S3-очистка включается только сочетанием `RECORDING_S3_DELETION_ENABLED=true` и `RECORDING_S3_AUTOMATIC_CLEANUP_ENABLED=true`; по умолчанию оба флага выключены. Scheduler также требует свежую завершённую reconciliation без активных проблем.
- Playback и локальная очистка считают S3-копией только объект со статусом `VERIFIED`. Активные analytics jobs, подготовка playback и export блокируют удаление записи; legacy-каталоги, путь которых не заканчивается на `recordingId`, автоматически не удаляются. Upload идемпотентен по `s3_key`, а глобальные операции S3 координируются PostgreSQL advisory locks.
...
GitHub Actions workflow [video-ingest-agent.yml](.github/workflows/video-ingest-agent.yml) запускается при изменениях агента, `stream-service`, MediaMTX или demo Compose. Он выполняет Rust `check`/`test`/Clippy, тесты `stream-service` и MediaMTX smoke-тест. Workflow можно временно отключить в разделе **Actions** репозитория или убрав/изменив его triggers.
